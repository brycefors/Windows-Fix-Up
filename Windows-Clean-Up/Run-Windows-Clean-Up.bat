...
:: This batch file is a simple wrapper to execute the Windows-Clean-Up.ps1 PowerShell script.
...
set "PS_SCRIPT=%SCRIPT_DIR%Windows-Clean-Up.ps1"

:: Check if the script exists. If not, download it.
IF NOT EXIST "%PS_SCRIPT%" (
    ECHO Windows-Clean-Up.ps1 not found. Downloading from GitHub...
    powershell -Command "Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/brycefors/Windows-Fix-Up/refs/heads/main/Windows-Clean-Up/Windows-Clean-Up.ps1' -OutFile '%PS_SCRIPT%'"
    IF NOT EXIST "%PS_SCRIPT%" (
        ECHO Failed to download Windows-Clean-Up.ps1.
        PAUSE
        EXIT /B 1
    )
