...
:: This batch file is a simple wrapper to execute the Windows-InPlace-Upgrade.ps1 PowerShell script.
...
set "PS_SCRIPT=%SCRIPT_DIR%Windows-InPlace-Upgrade.ps1"

:: Check if the script exists. If not, download it.
IF NOT EXIST "%PS_SCRIPT%" (
    ECHO Windows-InPlace-Upgrade.ps1 not found. Downloading from GitHub...
    powershell -Command "Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/brycefors/Windows-Fix-Up/refs/heads/main/Windows-InPlace-Upgrade/Windows-InPlace-Upgrade.ps1' -OutFile '%PS_SCRIPT%'"
    IF NOT EXIST "%PS_SCRIPT%" (
        ECHO Failed to download Windows-InPlace-Upgrade.ps1.
        PAUSE
        EXIT /B 1
    )
