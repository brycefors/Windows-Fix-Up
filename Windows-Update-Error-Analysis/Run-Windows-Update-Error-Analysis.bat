...
:: This batch file is a simple wrapper to execute the Get-WindowsUpdateErrorAnalysis.ps1 PowerShell script.
...
set "PS_SCRIPT=%SCRIPT_DIR%Get-WindowsUpdateErrorAnalysis.ps1"

:: Check if the script exists. If not, download it.
IF NOT EXIST "%PS_SCRIPT%" (
    ECHO Get-WindowsUpdateErrorAnalysis.ps1 not found. Downloading from GitHub...
    powershell -Command "Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/brycefors/Windows-Fix-Up/refs/heads/main/Windows-Update-Error-Analysis/Get-WindowsUpdateErrorAnalysis.ps1' -OutFile '%PS_SCRIPT%'"
    IF NOT EXIST "%PS_SCRIPT%" (
        ECHO Failed to download Get-WindowsUpdateErrorAnalysis.ps1.
        PAUSE
        EXIT /B 1
    )
